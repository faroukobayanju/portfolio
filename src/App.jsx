...
function Chevron({ up = false }) {
  return <svg className="inline-icon" viewBox="0 0 24 24" aria-hidden="true"><path d={up ? "m6 15 6-6 6 6" : "m6 9 6 6 6-6"} /></svg>;
}
...
function WorkCard({ item, index }) {
  ...
  return <article className={`work-card ${index % 3 === 0 ? "clipped" : "pin"}`} data-category={item.category}>
    ...
    <div className="card-content"><p className="handwritten category">{item.type}</p><h3>{item.title}</h3><p>{item.description}</p>
      ...
      {item.link ? <a className="text-link" href={href} {...externalProps(item.link)}>{item.cta || "view work"} <ArrowUpRight /></a> : null}
    </div>
  </article>;
}
...
export default function App() {
  ...
  const visibleWork = showAllWork ? filteredWork : filteredWork.slice(0, 6);
  const selectFilter = (name) => { setFilter(name); setShowAllWork(false); };
  ...
  return <>
    ...
    <main className="board">
      ...
      <section id="work" className="work-section"><div className="section-heading"><h2>selected work <span className="handwritten">from the browser tabs</span></h2><span className="round-stamp" aria-hidden="true">✎</span></div><p className="section-lede">things i’ve written, measured, and put into the world.</p><div className="filters" role="group" aria-label="filter selected work">{filters.map((name) => <button key={name} type="button" aria-pressed={filter === name} onClick={() => selectFilter(name)}>{name === "coding" ? "vibecoding" : name === "all" ? "everything" : name}</button>)}</div><p className="sr-only" role="status">{visibleWork.length} of {filteredWork.length} work samples shown.</p><div className="work-grid" aria-live="polite">{visibleWork.map((item, index) => <WorkCard key={item.id} item={item} index={index} />)}</div>{filteredWork.length > 6 ? <div className="work-reveal"><button className="reveal-button" type="button" aria-expanded={showAllWork} onClick={() => setShowAllWork((current) => !current)}>{showAllWork ? "show less" : `see more (${filteredWork.length - 6})`} <Chevron up={showAllWork} /></button></div> : null}</section>
