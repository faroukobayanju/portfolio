...
const definitions = {
  ...
  projects: {
    title: "off the clock", description: "side projects and experiments.", newLabel: "new project", saveLabel: "save project",
    fields: [["title", "title", "text", true], ["link", "link"], ["description", "description", "textarea", true, null, "span-all"], ["cta", "link text"], ["image", "image path or url"], ["imageFile", "choose a new image", "file", false, null, "span-all"], ["imageAlt", "image alt text", "text", false, null, "span-all"]]
  }
};
...
export default function Admin() {
  ...
  const saveItem = (section, item) => { const items = [...data[section]]; const index = items.findIndex((entry) => entry.id === item.id); if (index >= 0) items[index] = item; else items.push(item); return commit({ ...data, [section]: items }, `${section === "projects" ? "project" : section} published.`); };
  ...
  return <><header className="admin-header"><div><p className="handwritten">the control room</p><h1>portfolio admin</h1><p>edit the public page without touching the code.</p></div><a className="preview-link" href="index.html">view portfolio ↗</a></header><aside className="storage-note"><div><strong>changes publish to the live portfolio.</strong><span> enter the private admin password before saving, importing, removing, or restoring content.</span></div><label className="admin-password">admin password<input type="password" autoComplete="current-password" value={password} onChange={(event) => setPassword(event.target.value)} placeholder="enter password" /></label></aside><main className="admin-main"><nav className="admin-tabs" aria-label="admin sections">{[["about", "about"], ["work", "selected work"], ["experience", "experience"], ["projects", "off the clock"]].map(([id, label]) => <button type="button" key={id} aria-pressed={tab === id} onClick={() => setTab(id)}>{label}</button>)}</nav>{tab === "about" ? <AboutPanel about={data.about} onSave={saveAbout} /> : <CrudPanel section={tab} items={data[tab]} onSave={saveItem} onRemove={removeItem} />}</main><footer className="admin-tools"><div><strong>backup & restore</strong><p>download your entries as json or bring a previous export back.</p></div><div className="tool-actions"><button type="button" onClick={exportJson}>export json</button><label className="import-label">import json<input type="file" accept="application/json" onChange={loadJson} /></label><button type="button" onClick={restore}>restore defaults</button></div></footer>{toast ? <div className="toast" role="status" aria-live="polite"><span>{toast}</span></div> : null}</>;
}
